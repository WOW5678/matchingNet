    ...
    def run_val_epoch(self, total_val_batches):
        ...
        with tqdm.tqdm(total=total_val_batches) as pbar:
            for i in range(total_val_batches):
                ...
                x_support_set = Variable(torch.from_numpy(x_support_set)).float()
                y_support_set = Variable(torch.from_numpy(y_support_set), requires_grad=False).long() # (20,20)
                x_target = Variable(torch.from_numpy(x_target)).float() #(20,20,28,28,1)
                y_target = Variable(torch.from_numpy(y_target), requires_grad=False).squeeze().long()#shape:[20]

                # convert to one hot encoding
                y_support_set = y_support_set.unsqueeze(2) #shape:(20,20,1) 即（batch_size,sequence_length,1）
                sequence_length = y_support_set.size()[1]
                batch_size = y_support_set.size()[0]
                y_support_set_one_hot = Variable(
                ...
                # shape:(20,20,20) 即（batch_size, sequence_length, self.classes_per_set）

                # reshape channels and change order
                size = x_support_set.size()
                print('x_support_set.size:',size) #torch.Size([20, 20, 28, 28, 1])
                x_support_set = x_support_set.permute(0, 1, 4, 2, 3)
                print('x_support_set_permute:',x_support_set.size()) #torch.Size([20, 20, 1, 28, 28])
                x_target = x_target.permute(0, 3, 1, 2) #（20,1,28,28）
                print('x_target.size:',x_target.size()) #x_target.size: torch.Size([20, 1, 28, 28])即（batch_size,channels,weight,height）
                total_accuracy += acc.data[0]
                if self.isCuadAvailable & self.use_cuda:
                    acc, c_loss = self.matchNet(x_support_set.cuda(), y_support_set_one_hot.cuda(), x_target.cuda(),
                                                y_target.cuda())
